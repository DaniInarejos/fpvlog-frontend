...
<template>
  ...
  >
    <div class="space-y-6">
      ...
      <div class="space-y-4">
        <div>
          <h3 class="text-lg font-medium text-gray-900">{{ t('message.drones.description') }}</h3>
          <dl class="mt-2 grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div class="flex justify-between sm:block">
              <dt class="text-sm font-medium text-gray-500">{{ t('message.drones.form.brand') }}</dt>
              <dd class="text-sm text-gray-900">{{ drone.brand?.name || '-' }}</dd>
            </div>
            <div class="flex justify-between sm:block">
              <dt class="text-sm font-medium text-gray-500">{{ t('message.drones.form.model') }}</dt>
              <dd class="text-sm text-gray-900">{{ drone.model || '-' }}</dd>
            </div>
            <div class="flex justify-between sm:block">
              <dt class="text-sm font-medium text-gray-500">{{ t('message.drones.form.weight') }}</dt>
              <dd class="text-sm text-gray-900">{{ drone.weight ? `${drone.weight}g` : '-' }}</dd>
            </div>
            <div class="flex justify-between sm:block">
              <dt class="text-sm font-medium text-gray-500">{{ t('message.drones.form.frame') }}</dt>
        ...
        <div class="flex items-center gap-4 text-sm text-gray-600">
          <div class="flex items-center gap-1">
            ...
            {{ drone.likesCount || 0 }} {{ t('message.common.likes') }}
          </div>
        </div>
      </div>
    </div>
